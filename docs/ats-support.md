...
| **Workable**              | ✅ supported  | ✅         | ✅         | Host: `apply.workable.com/<slug>`. API: `GET /api/v1/widget/accounts/<slug>`. Slug variants: `<slug>-careers`, `<slug>hq`, `<slug>-hq`. `body` is empty — public widget payload has no job description.           |
